...
actions.configureDestinationDir = function() {
  ...
  function processDir(dir) {
    ...
    var root = path.join(self.destinationRoot(), dir);
    if (!fs.existsSync(root)) {
      self.log.create(dir + '/');
      fs.mkdirSync(root);
    }
    self.destinationRoot(root);
    self.log.info(g.f('Setting the working directory to %s', dir));
  }

  if (this.options['config-file'] && this.configFile.dirName) {
    var dir = self.configFile.dirName;
    processDir(dir);
    done();
  } else {
    return this.prompt([
